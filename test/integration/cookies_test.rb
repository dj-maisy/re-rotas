...
class CookiesTest < ActionDispatch::IntegrationTest
  test "cookies are secure" do
    ...
    assert_not_nil cookies[GdsRotas::CookieNames::SESSION_COOKIE_NAME]

    # need to use response.headers to verify that SameSite and expiry are set :/
    assert_match(/^#{GdsRotas::CookieNames::SESSION_COOKIE_NAME}=.+$/, response.headers["Set-Cookie"])

    assert_match(/SameSite=Lax/i, response.headers["Set-Cookie"], "SameSite=Lax not enabled for this app!")

    assert_match(/HttpOnly/i, response.headers["Set-Cookie"], "HttpOnly not enabled for this app!")
